...
for i=1:size(ahix,1)
    ...
    ss=S1{i};ss1=S{i};
    [~, ~, y1 ,MAE, RMSE] = osares(leng,ss,ss1,yy20,y20,osacha,bmi,leng);
    s=[s;S{i}];
    y=[y;y1]; 
    Y20=[Y20;y20];
    YY20=[YY20;yy20];
...
x=x*100;y=y*100;
% regressionTree = fitrtree(train_x, train_y, 'MinLeafSize', 1, 'Surrogate', 'off');
% % regressionTree = crossval(regressionTree, 'KFold', 5);
% % validationRMSE = sqrt(kfoldLoss(partitionedModel, 'LossFun', 'mse'));
% y = predict(regressionTree, test_x);
% x=predict(regressionTree,train_x);
MAE=mean(test_y-y);
...
% da2=[0 da2];
da3=find(da2>1);
if size(da3,1)>size(da3,2)
da3=da3';
end
da3=[ 0 da3 length(da1)];
da4=diff(da3);
da5=[];l1=1;l2=0;
for k1=1:length(da4)
    le=da4(k1);
    if le<=3
        ...
        da5=[da5 [2*ones(1,floor(le/2)) rem(le,2)]];
    end
end
da5=da5(da5~=0);
clear da3 da2 da1
...
osadatr(i,1)=length(find(da==1))/(sleepeff(i)/(length(da)/2)/60);
osadatr(i,2)=length(find(da==2))/(sleepeff(i)/(length(da)/2)/60);%length(da);

% osadatr(i,1)=length(da5)/(sleepeff(i)/100*(length(da)/2)/60);
% osadatr(i,2)=sum(da4)/length(da5)/2;
% osadatr(i,3)=length(find(da5==1))/length(da5);
% osadatr(i,4)=length(find(da5==2))/length(da5);

% osadatr(i,7)=std(da4);

% osadatr(i,3)=length(find(diff(da)~=0))/(sleepeff(i)/(length(da)/2)/60);
% osadatr(i,4)=length(find(diff(da)==0))/(sleepeff(i)/(length(da)/2)/60);
...
osatr=osacha(ss)/100;
% n=[0 5 15 30];osatr1=ones(size(osatr,1),1);
% for i=1:length(n)-1
%     n1=n(i);n2=n(i+1);
%     osatr1(intersect(find(osatr>=n1),find(osatr<n2)))=(i+1);
% end
end
